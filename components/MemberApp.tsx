...
export function MemberApp() {
  ...
  const todayMission = { ...baseMission, minutes: Math.min(baseMission.minutes, availableMinutes), action: `${baseMission.action} Faça isso com foco em ${focus.toLowerCase()}, dentro do tempo que cabe na sua rotina.` };
...
function Journey({ completedIds, currentDay, maxDays, points, focus, availableMinutes, onComplete }: { completedIds: Set<string>; currentDay: number; maxDays: number; points: number; focus: string; availableMinutes: number; onComplete: (id: string, response?: string) => Promise<void> }) {
  const availableMissions = missions.slice(0, maxDays);
  const [selectedId, setSelectedId] = useState(availableMissions[Math.min(currentDay - 1, availableMissions.length - 1)]?.id || "dia-1");
  const selectedBase = availableMissions.find(item => item.id === selectedId) || availableMissions[0];
  const selected = { ...selectedBase, minutes: Math.min(selectedBase.minutes, availableMinutes), action: `${selectedBase.action} Relacione esse movimento com ${focus.toLowerCase()}.` };
