...
const empty: Snapshot = {
  ...
  preferences: { reminders_enabled: 1, marketing_enabled: 0, theme: "light", text_size: "normal" }, products: [], posts: [], likedPostIds: [], notifications: [],
};
...
function expandedAccess(access: Set<string>) {
  const expanded = new Set(access);
  if (access.has("completo")) ["mapa", "sos", "desafio"].forEach(id => expanded.add(id));
  if (access.has("jornada")) ["mapa", "sos", "desafio"].forEach(id => expanded.add(id));
  return expanded;
}
...
function canOpen(tab: Tab, access: Set<string>) {
  ...
  if (tab === "journey") return hasAny(access, ["mapa", "desafio", "completo", "jornada"]);
  if (tab === "sos") return hasAny(access, ["sos", "completo", "jornada"]);
  if (tab === "journal") return hasAny(access, ["mapa", "completo", "jornada"]);
...
function nextProduct(products: Product[], access: Set<string>, completed = 0) {
  const available = products.filter(product => !access.has(product.id));
  return available.find(product => product.id !== "jornada") || (completed >= 7 ? available.find(product => product.id === "jornada") : undefined);
}
...
export function MemberApp() {
  ...
  const access = useMemo(() => expandedAccess(new Set(data.access.filter(row => row.status === "active").map(row => String(row.product_id)))), [data.access]);
  const completedIds = useMemo(() => new Set(data.missions.filter(m => m.status === "completed").map(m => String(m.mission_id))), [data.missions]);
  const maxDays = access.has("jornada") ? 30 : hasAny(access, ["mapa", "desafio", "completo"]) ? 7 : 0;
  ...
  async function dismissNotification(id: string) {
    try {
      await post({ action: "notification.read", id });
      setData(current => ({ ...current, notifications: current.notifications.filter(item => String(item.id) !== id) }));
    } catch (error) { say(error instanceof Error ? error.message : "Não foi possível fechar a novidade."); }
  }
  ...
  return <div className="app-shell">
    ...
    <main className="app-main"><header className="app-topbar"><div><span className="mobile-brand">V</span><p>{tab === "today" ? "Seu espaço de hoje" : nav.find(item => item[0] === tab)?.[1]}</p></div><div className="top-stats"><span><b>{streak}</b> dias</span><span><b>{points}</b> pontos</span>{data.notifications.length > 0 && <span className="notification-count" title="Novidades na sua jornada">● {data.notifications.length}</span>}<button className={large ? "active" : ""} onClick={toggleTextSize} aria-label={large ? "Voltar ao texto padrão" : "Aumentar texto"} title={large ? "Texto ampliado" : "Aumentar texto"}>{large ? "A−" : "A+"}</button><button onClick={() => setDark(!dark)} aria-label="Alternar modo escuro">◐</button></div></header>
      ...
      {loading ? <div className="app-loading"><i/><p>Preparando seu espaço…</p></div> : <>
        {data.notifications.length > 0 && <section className="member-notifications" aria-label="Novidades da sua jornada">{data.notifications.slice(0, 2).map(item => <article key={String(item.id)}><span>Novidade</span><div><strong>{String(item.title)}</strong><p>{String(item.body || "")}</p>{item.kind === "journey_ready" && data.products.find(product => product.id === "jornada") && <a href={offerHref(data.products.find(product => product.id === "jornada")!)}>Conhecer a Jornada de 30 dias</a>}</div><button type="button" onClick={() => dismissNotification(String(item.id))} aria-label="Fechar novidade">×</button></article>)}</section>}
...
function MemberWelcome({ data }: { data: Snapshot }) {
  ...
  const access = expandedAccess(new Set(data.access.filter(row => row.status === "active").map(row => String(row.product_id))));
  const completed = data.missions.filter(mission => mission.status === "completed").length;
  const recommended = nextProduct(data.products, access, completed);
  return <div className="app-view"><section className="welcome-row"><div><span className="eyebrow">Seu espaço individual</span><h1>Olá, {firstName(data.user.name)}.</h1><p>{profile ? `Seu diagnóstico indica o perfil ${profile.name}. Agora escolha o próximo passo que deseja liberar.` : "Faça o diagnóstico para receber uma recomendação alinhada à sua rotina."}</p></div></section>
    ...
    <section className="access-section journey-store"><div><span className="eyebrow">Ordem recomendada</span><h2>Uma etapa por vez, dentro da sua realidade.</h2><p>Você mantém tudo na mesma conta e escolhe quando deseja avançar.</p></div><div>{data.products.map((product, index) => { const copy = productJourney[product.id]; const acquired = access.has(product.id); const isRecommended = recommended?.id === product.id; const locked = product.id === "jornada" && completed < 7 && !acquired; return <article key={product.id} className={`${acquired ? "active" : ""} ${isRecommended ? "recommended" : ""}`}>
      <em>{acquired ? "Etapa liberada" : locked ? "Libera após seu primeiro ciclo" : isRecommended ? "Seu próximo passo" : `Etapa ${index + 1}`}</em><span>{index + 1}. {product.name}</span><small>{copy?.promise || product.access}</small><p>{locked ? "Conclua sete dias para receber esta recomendação no momento certo." : isRecommended ? personalizedOfferReason(profileKey) : copy?.reason}</p><b>{acquired ? "Acesso ativo" : locked ? "Ainda não recomendado" : `R$ ${product.price.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`}</b>{!acquired && !locked && <a className="button button-small" href={offerHref(product)}>{copy?.cta || "Quero este produto"}</a>}
    </article>; })}</div></section>
  </div>;
}
...
function NextStepOffer({ data, access }: { data: Snapshot; access: Set<string> }) {
  ...
  const completed = data.missions.filter(mission => mission.status === "completed").length;
  const product = nextProduct(data.products, access, completed);
...
function Today({ data, mission, day, maxDays, streak, points, completed, access, onComplete, post, refresh, say, onNavigate }: { data: Snapshot; mission: typeof missions[number]; day: number; maxDays: number; streak: number; points: number; completed: boolean; access: Set<string>; onComplete: (id: string, response?: string) => Promise<void>; post: (body: Row) => Promise<unknown>; refresh: () => Promise<void>; say: (message: string) => void; onNavigate: (tab: Tab) => void }) {
  ...
  const profile = data.personalization?.profileKey ? profileContent[data.personalization.profileKey] : null;
  const checkinAllowed = hasAny(access, ["desafio", "completo", "jornada"]);
...
function Community({ data, post, refresh, say }: { data: Snapshot; post: (body: Row) => Promise<unknown>; refresh: () => Promise<void>; say: (message: string) => void }) {
  ...
  useEffect(() => { try { setSaved(new Set(JSON.parse(localStorage.getItem("volta-saved-posts") || "[]"))); } catch { setSaved(new Set()); } }, []);
  function toggleSaved(id: string) { const next = new Set(saved); if (next.has(id)) next.delete(id); else next.add(id); setSaved(next); localStorage.setItem("volta-saved-posts", JSON.stringify([...next])); say(next.has(id) ? "Publicação salva neste dispositivo." : "Publicação removida dos itens salvos."); }
