...
export function ResultPage() {
  ...
  return <main className="result-shell">
    ...
    <section className="offer-section"><div className="offer-copy"><span className="offer-tag">Próximo passo recomendado</span><span className="eyebrow">{mapa?.name || "Mapa da Volta"}</span><h2>Transforme este resultado em sete dias de ação.</h2><p>Seu plano será associado à sua conta e adaptado ao perfil, à área escolhida e ao tempo que cabe na sua rotina.</p><ul><li>Diagnóstico completo e mapa das 8 áreas</li><li>Plano personalizado de 7 dias</li><li>Uma missão curta por dia</li><li>Checklist e diário guiado</li><li>Relatório individual no celular</li></ul></div><div className="checkout-card"><span className="test-badge">Acesso individual</span><p>{mapa?.name || "Mapa da Volta"}</p><div className="price"><small>R$</small><strong>{Math.floor(mapa?.price || 17)}</strong><small>,00</small></div><label className="order-bump"><input type="checkbox" checked={bump} onChange={event => setBump(event.target.checked)}/><span><b>Quero adicionar o {sos?.name || "Kit SOS"}</b><small>Respostas práticas para dias difíceis por R$ {(sos?.price || 27).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}.</small></span></label><div className="total"><span>Total</span><strong>R$ {total.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</strong></div><a className="button" href={checkoutHref(mapa, data, bump)}>{mapa?.checkoutUrl ? "Continuar para o pagamento" : "Entrar para continuar"}</a><p className="secure-note">✓ O acesso é liberado somente para a conta da compradora<br/>✓ Pagamentos aprovados são confirmados automaticamente</p></div></section>
