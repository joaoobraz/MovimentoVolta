...
function QuizAnalytics({ data }: { data: AdminData }) {
  ...
  const stages = [
    ...
    ["Pagamentos aprovados", funnel.stages.purchases, "Total geral dos últimos 30 dias"],
  ] as const;
  const route = [
  ...
  return <div className="admin-view quiz-analytics-view">
    ...
    <div className="quiz-analytics-layout"><article className="question-funnel-card"><div className="section-title-row"><div><span className="eyebrow">24 perguntas</span><h2>Avanço pergunta por pergunta</h2></div><span>Últimos {funnel.periodDays} dias</span></div><div className="question-funnel-list">{questions.map(item => { const rate = visitors ? Math.round(item.answered / visitors * 100) : 0; return <div className="question-funnel-row" key={item.question.id}><span>{String(item.index).padStart(2, "0")}</span><div><strong>{item.question.text}</strong><small>{item.viewed} visualizaram · {item.answered} responderam · {item.abandoned} saíram nesta etapa</small><i><b style={{ width: `${Math.min(100, rate)}%` }}/></i></div><em>{rate}%</em></div>; })}</div></article><aside className="source-funnel-card"><span className="eyebrow">Origem</span><h2>De onde chegaram</h2>{funnel.sources.length ? funnel.sources.map(item => <div className="source-funnel-row" key={item.source}><div><strong>{friendlySource(item.source)}</strong><span>{item.total}</span></div><i><b style={{ width: `${item.total / maximumSource * 100}%` }}/></i></div>) : <div className="empty-state compact"><p>As origens aparecerão nas próximas visitas.</p></div>}<div className="historical-funnel-note"><strong>Histórico anterior</strong><p>Antes desta medição detalhada, o sistema registrou {data.metrics.quizStarted} carregamentos do quiz, {data.metrics.quizCompleted} diagnósticos concluídos e {data.metrics.checkoutClicked} cliques no checkout.</p></div></aside></div>
