...
function QuizAnalytics({ data }: { data: AdminData }) {
  ...
  const finalStops = new Map((funnel.lastReached || []).map(item => [item.questionIndex, item.total]));
  ...
  return <div className="admin-view quiz-analytics-view">
    ...
    <section className="quiz-reach-card">
      <div className="section-title-row"><div><span className="eyebrow">Leitura rápida</span><h2>Até qual número do quiz elas foram</h2><p>Cada bloco mostra quantas sessões chegaram à pergunta e quantas terminaram com ela como último número registrado.</p></div><span>1 a 24</span></div>
